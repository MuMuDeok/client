...
class MyCalendarTapViewModel {
    ...
    func getLoopCount(date: [Date]) -> Int {
    ...
    func getCurrentMonthAllDate(month: Date) -> [[Date]] {
        ...
        var previousMonthComponent = calendar.dateComponents([.year, .month], from: changeMonth(month: month, value: -1))
        var nextMonthComponent = calendar.dateComponents([.year, .month], from: changeMonth(month: month, value: 1))
        
        var dayArr: [Date] = []
        
        for i in 1...weekCount * 7 {
            if i < firstWeekday { // 저번 달
                let day = dayNumberOfMonth(month: changeMonth(month: month, value: -1)) - firstWeekday + i
    ...
    func getWeekData(date: [Date]) -> ([CalendarDayEvents], [[Bool]], Int) {
        let events: [any Event] = getWeekEvents(date: date)
        let loopCount: Int = getLoopCount(date: date)
